...
namespace Pan.Affiliation.Infrastructure.Persistence.Entities
{
    ...
    {
        [MaxLength(255), Required] public string? Name { get; set; }

        [MaxLength(14), Required] public string? DocumentNumber { get; set; }
        ...
        public static Customer FromDomainEntity(Domain.Modules.Customers.Entities.Customer customer) =>
            new()
            {
                ...
                Id = customer.Id,
                Addresses = customer.Addresses.Select(a => Address.FromDomainEntity(a, customer))
            };
    }
}