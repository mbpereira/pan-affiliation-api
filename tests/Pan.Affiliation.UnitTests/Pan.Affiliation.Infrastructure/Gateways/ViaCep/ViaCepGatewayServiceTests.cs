...
public class ViaCepGatewayServiceTests
{
    ...
    [Fact]
    public async Task When_GetPostalCodeInformationAsync_Called_should_throws_exception_if_http_response_is_not_success()
    {
        var viaCepGatewayService = HttpClientFactoryUtils.CreateMockedHttpClientFactory(
            factory => new ViaCepGatewayService(factory, _settingsProvider),
            ViaCepClient,
